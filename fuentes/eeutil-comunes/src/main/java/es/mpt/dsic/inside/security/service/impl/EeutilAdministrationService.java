...
import es.mpt.dsic.inside.model.EeutilAuditoria;
...
public class EeutilAdministrationService implements AdministrationService {
  ...
  @Override
  public void capturarAuditoria(AuditoriaObject peticion) {
    EeutilAuditoria auditoriaDatabase = new EeutilAuditoria();
    auditoriaDatabase.setAplicacion(peticion.getAplicacion());
    auditoriaDatabase.setOperacion(peticion.getOperacion());
    auditoriaDatabase.setFecha(peticion.getFecha());
    auditoriaDatabase.setServicio(peticion.getServicio());
    eeutilDao.salvar(auditoriaDatabase);
  }
