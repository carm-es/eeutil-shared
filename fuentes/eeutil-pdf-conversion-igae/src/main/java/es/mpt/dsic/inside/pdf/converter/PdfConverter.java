...
import es.mpt.dsic.inside.pdf.converter.utilsigae.RestIgaeServiceA;
import es.mpt.dsic.inside.pdf.rest.libreoffice.ClientSecureRestLibreofficeConvIgae;
import es.mpt.dsic.inside.pdf.rest.libreoffice.ConfigureRestInfoConvIgae;
...
public class PdfConverter {
  ...
  protected static final Log logger = LogFactory.getLog(PdfConverter.class);
  ...
  @Autowired
  RestIgaeServiceA restIgaeServiceA;

  @Autowired
  ClientSecureRestLibreofficeConvIgae clientSecureRestLibreofficeConvIgae;
    ...
    try {
      ...
      if (mime.contentEquals("text-tcn/html") || mime.contentEquals("text/tcn")) {
        ...
        try {
          ...
          response = restIgaeServiceA.getResponseClientIgae(httpClient,
              baseUrlIgae + "/api/convertirTCN", bContenido, tokenNameRestIgae, tokenValueRestIgae);

          HttpEntity entityResponse = response.getEntity();

          if (response.getStatusLine().getStatusCode() != 200) {
            throw new EeutilException(ERROR_AL_ACCEDER_AL_SERVICIO_DE_IGAE + baseUrlIgae + STATUS
          ...
          restIgaeServiceA.cerrarRequestResponse(httpClient, response);
        }
        ...
        // si falla la conversion por igae hacemos la conversion normal
          ...
          if (!ConfigureRestInfoConvIgae.isActiveNewLibreoffice()) {
  ...
  private File convertirRest(File contenido, String mimeType) throws EeutilException {
    ...
    try {
      ...
      String urlBaseRestLibreoffice = ConfigureRestInfoConvIgae.getBaseUrlSecureLibreoffice();
      String nameTokenLibreoffice = ConfigureRestInfoConvIgae.getTokenNameLibreoffice();
      String valueTokenLibreoffice = ConfigureRestInfoConvIgae.getTokenValueLibreoffice();

      response = clientSecureRestLibreofficeConvIgae.getResponseClientLibreoffice(httpClient,
      ...
      try {
        clientSecureRestLibreofficeConvIgae.cerrarRequestResponse(httpClient, response);
      } catch (IOException e) {
        throw new EeutilException(e.getMessage(), e);
      }
    }
